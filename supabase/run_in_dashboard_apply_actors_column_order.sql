-- HOW TO RUN (read this — the Dashboard runs SQL only, not filenames):
-- 1. Open THIS file in Cursor/VS Code (double-click it in the file tree).
-- 2. Select all (Cmd+A / Ctrl+A), copy, paste into Supabase → SQL Editor.
-- 3. Do NOT type or paste the path "supabase/run_in_dashboard_apply_actors_column_order.sql"
--    into SQL Editor — that causes ERROR 42601 near "supabase".
--
-- Reorders public.actors: ethnicity → sex → height → weight → …
-- Run once only; re-running fails (relation actors already exists).
-- Mirrors migration 20260411170000_actors_column_order_casting_after_ethnicity.sql
